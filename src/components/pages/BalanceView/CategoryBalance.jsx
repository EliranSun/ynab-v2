...
export const CategoryBalance = ({categoryId, categoryName, currentTimestamp, isSameDate, isPreviousMonth}) => {
    ...
    return (
        <div className="bg-gray-200 px-2 pb-2 w-full md:w-fit">
            <div className="flex justify-between my-2" onClick={() => setIsExpanded(!isExpanded)}>
                <span className="text-xl font-bold">{categoryName}</span>
                <span className="text-xl font-bold">{formatCurrency(totalExpensesSum, false)}</span>
            </div>
            {isExpanded ? <div className="flex gap-2 min-w-fit flex-wrap items-stretch">
