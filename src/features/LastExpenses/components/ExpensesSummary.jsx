...
export const ExpensesSummary = ({ budget = {}, expenses = [] }) => {
	...
	const lastItems = useMemo(() => {
		...
		for (const item of expenses) {
			const outOfRange =
				isBefore(item.timestamp, startDate) || isAfter(item.timestamp, endDate);
			const itemIsFiltered = itemsHiddenByUser.some(
		...
		if (sortBy === "timestamp") {
			return Object.values(expensesFoo).sort(
	...
	const incomeAmountForTimeframe = useMemo(() => {
		const total = incomeForTimeframe.reduce(
	...
	const totalSpent = useMemo(
		...
		[incomeSubcategoriesIds, lastItems]
	);

	const hiddenItemsAmountSum = useMemo(() => {
		return formatCurrency(
			itemsHiddenByUser.reduce((acc, item) => {
				return acc + item.amount;
			}, 0),
			false,
			false
									...
									return (
										<div
											...
											<Expense
												...
												budget={
													budget.find(
														(b) => b.subcategoryId === item.subcategoryId
													)?.amount
												}
												isIncome={incomeSubcategoriesIds.includes(
													item.subcategoryId
												)}
												onHide={() => {
													const isHidden = itemsHiddenByUser.some(
														({ id }) => id === item.id
													);
													if (isHidden) {
														setItemsHiddenByUser(
															itemsHiddenByUser.filter(
																({ id }) => id !== item.id
															)
														);
														return;
