...
export const supabase = process.env.REACT_APP_SUPABASE_URL
	? createClient(
...
export const getBudget = async () => {
	const { data, error } = await supabase
		.from("budget")
		.select(
			`*, subcategory:subcategories (name, icon, category:categories (isIncome))`
		);
...
export const updateBudget = async ({ id, amount, subcategoryId }) => {
	...
	if (id) {
		...
		const response = await supabase.from("budget").insert({
			amount,
			subcategoryId,
		});
